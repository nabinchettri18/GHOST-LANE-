...
const required = (v: string, field: string, row: number) => {
  const value = text(v)
  if (!value) throw new Error(`${field} is required on row ${row}`)
  return value.slice(0, 500)
}
const numberOrNull = (v: string, field: string, row: number) => {
  const value = text(v)
  if (!value) return null
  const parsed = Number(value.replace(/,/g, ''))
  if (!Number.isFinite(parsed)) throw new Error(`${field} must be numeric on row ${row}`)
  return parsed
}
const bounded = (v: string, field: string, row: number) => {
  const n = numberOrNull(v, field, row)
...
async function refreshLaneVolumes(db: ReturnType<typeof createAdminClient>, org: string, ids: string[], contracts: boolean, shipments: boolean) {
  ...
  for (const row of shipmentResult.data ?? []) {
    if (rejectedStatuses.has(text(String(row.status ?? '')).toLowerCase())) continue
    moved.set(row.lane_id, (moved.get(row.lane_id) ?? 0) + Math.max(0, Number(row.volume ?? 0)))
  }
  for (const id of unique) {
    ...
    if (Object.keys(patch).length) {
      const { error } = await db.from('lanes').update(patch).eq('id', id).eq('organization_id', org)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!dataRows.length) throw new Error('No data rows were found after the header')

    const db = createAdminClient()
    const organizationId = membership.organization_id
    const { data: existingLanes, error: laneError } = await db.from('lanes').select('id,origin,destination').eq('organization_id', organizationId)
    if (laneError) throw laneError
    const laneMap = new Map((existingLanes ?? []).map(lane => [laneKey(lane.origin, lane.destination), lane.id]))
    const warnings = [...validation.optionalMissing]
    let inserted = 0
    let skipped = 0
    const affectedLanes: string[] = []
    ...
    if (kind === 'lanes') {
      ...
      const payload = dataRows.map((row, i) => {
        ...
        return {
          organization_id: organizationId, origin, destination,
          mode: text(value(row, 'mode')).slice(0, 100) || null,
      ...
      if (payload.length) {
        ...
        if (error) throw new Error(`Lane import failed: ${error.message}`)
        inserted = payload.length
      }
      skipped = dataRows.length - inserted
    } else {
      // Contracts and shipments may introduce lanes. Master shipment exports are
      // intentionally treated as shipments by the UI, so extra master columns are safe.
      ...
      if (missing.size) {
        const { data: created, error } = await db.from('lanes').insert([...missing.values()].map(lane => ({ organization_id: organizationId, ...lane, mode: 'road', contracted_volume: 0, materialized_volume: 0 }))).select('id,origin,destination')
        if (error) throw new Error(`Lane creation failed: ${error.message}`)
        for (const lane of created ?? []) laneMap.set(laneKey(lane.origin, lane.destination), lane.id)
      }

      if (kind === 'contracts') {
        const ids = dataRows.map((row, i) => required(value(row, 'contract_id'), 'contract_id', i + 2))
        const { data: existing } = await db.from('contracts').select('contract_id').eq('organization_id', organizationId).in('contract_id', ids)
        const existingIds = new Set((existing ?? []).map(row => row.contract_id))
        const payload = dataRows.map((row, i) => {
          const contractId = ids[i]
          if (existingIds.has(contractId)) return null
          const laneId = laneMap.get(laneKey(value(row, 'origin'), value(row, 'destination')))
          if (!laneId) throw new Error(`Lane could not be resolved on row ${i + 2}`)
          return {
            organization_id: organizationId, contract_id: contractId, lane_id: laneId,
        ...
        if (payload.length) {
          ...
          if (error) throw new Error(`Contract import failed: ${error.message}`)
        }
        inserted = payload.length
        skipped = dataRows.length - inserted
        for (const row of dataRows) {
          const id = laneMap.get(laneKey(value(row, 'origin'), value(row, 'destination')))
          if (id) affectedLanes.push(id)
        }
        await refreshLaneVolumes(db, organizationId, affectedLanes, true, false)
      } else {
        const ids = dataRows.map((row, i) => required(value(row, 'shipment_id'), 'shipment_id', i + 2))
        const { data: existing } = await db.from('shipments').select('shipment_id').eq('organization_id', organizationId).in('shipment_id', ids)
        const existingIds = new Set((existing ?? []).map(row => row.shipment_id))
        const payload = dataRows.map((row, i) => {
          const shipmentId = ids[i]
          if (existingIds.has(shipmentId)) return null
          const laneId = laneMap.get(laneKey(value(row, 'origin'), value(row, 'destination')))
          if (!laneId) throw new Error(`Lane could not be resolved on row ${i + 2}`)
          ...
          return {
            organization_id: organizationId, shipment_id: shipmentId, lane_id: laneId,
            carrier: required(value(row, 'carrier'), 'carrier', i + 2),
            shipment_date: text(value(row, 'shipment_date')) || null,
            volume: Math.max(0, Math.round(numberOrNull(value(row, 'volume'), 'volume', i + 2) ?? 0)),
            status, expected_cost: expectedCost, actual_cost: actualCost,
            expected_transit_hours: expectedTransit, actual_transit_hours: actualTransit,
            eta_date: text(value(row, 'eta_date')) || null, delivered_at: text(value(row, 'delivered_at')) || null,
            exception_type: text(value(row, 'exception_type')).slice(0, 200) || null,
            risk_score: risk,
            notes: text(value(row, 'notes')).slice(0, 2000) || null,
            ghost_lane_score: ghostScore, ghost_lane_status: ghostStatus,
            ghost_lane_reason: ghostReason.slice(0, 500), ghost_lane_confidence: confidence,
          }
        }).filter(Boolean)
        if (payload.length) {
          const { error } = await db.from('shipments').insert(payload)
          if (error) throw new Error(`Shipment import failed: ${error.message}`)
        }
        inserted = payload.length
        skipped = dataRows.length - inserted
        for (const row of dataRows) {
    ...
    return NextResponse.json({
      ...
      warnings,
      message: skipped ? `${inserted} imported, ${skipped} duplicate rows skipped.` : `${inserted} records imported successfully.`,
    })
  } catch (error) {
