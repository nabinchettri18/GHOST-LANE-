...
export default function ImportPage() {
  ...
  async function inspectFile(file: File): Promise<QueueItem> {
    ...
    try {
      ...
      return { file, kind, rows: parsed.length - 1, missing: validation.missing, optionalMissing: validation.optionalMissing }
    } catch {
  ...
  function updateKind(index: number, kind: ImportKind) {
    setItems(current => current.map((item, i) => {
      ...
      const validation = item.rows && item.file.name.toLowerCase().match(/\.(csv|tsv|txt|json|jsonl|ndjson)$/)
        ? validateHeaders(parseCsv('')[0] ?? [], kind)
        : { missing: [], optionalMissing: [] }
      return { ...item, kind, missing: validation.missing, optionalMissing: validation.optionalMissing }
    }))
  }
  ...
  async function importAll() {
    ...
    // Full navigation forces the server component to fetch the newly imported rows.
    ...
    <main className="min-h-screen bg-[#f7f9fc] px-4 py-7 text-[#08111f] sm:px-8 sm:py-10">
      <div className="mx-auto max-w-[1180px]">
        ...
        <div className="mt-5 border-b border-[#dbe2ec] pb-7">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[#66758a]">Upload one master shipment export or multiple operational files. GhostLane detects the dataset type, validates the required fields, prevents duplicate records and refreshes the dashboard after import.</p>
        </div>
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[1fr_330px]">
          <section className="rounded-3xl border border-[#dbe2ec] bg-white p-5 sm:p-7">
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-2xl bg-[#f6f8fb] p-4"><ShieldCheck size={18} className="text-[#1769e0]"/><p className="mt-3 text-xs font-extrabold">2. Validate</p><p className="mt-1 text-[10px] leading-4 text-[#66758a]">Required fields are checked before anything is written.</p></div>
              <div className="rounded-2xl bg-[#f6f8fb] p-4"><CheckCircle2 size={18} className="text-[#187650]"/><p className="mt-3 text-xs font-extrabold">3. Analyze</p><p className="mt-1 text-[10px] leading-4 text-[#66758a]">The overview reloads with your real workspace data.</p></div>
            </div>
            ...
            {items.length > 0 && <div className="mt-6 space-y-3">
              ...
              {items.map((item, index) => <div key={`${item.file.name}-${index}`} className="rounded-2xl border border-[#e1e7ef] p-4">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2"><select value={item.kind} onChange={e => updateKind(index, e.target.value as ImportKind)} className="rounded-lg border border-[#dbe2ec] bg-white px-3 py-2 text-[10px] font-bold outline-none"><option value="lanes">Lanes</option><option value="carriers">Carriers</option><option value="contracts">Contracts</option><option value="shipments">Shipments / Master</option></select><span className="rounded-lg bg-[#f6f8fb] px-3 py-2 text-[10px] font-bold text-[#66758a]">Detected from headers when possible</span></div>
                ...
                {!item.error && !item.missing.length && item.optionalMissing.length > 0 && <div className="mt-3 text-[10px] text-[#7a5a00]">Optional fields missing: {item.optionalMissing.slice(0, 5).join(', ')}{item.optionalMissing.length > 5 ? '…' : ''}</div>}
              </div>)}
            </div>}
