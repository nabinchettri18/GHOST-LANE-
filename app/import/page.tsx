...
export default function ImportPage() {
  ...
  async function onFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setRows(parsed)
    } catch { setError('The file could not be parsed. Check that it is a valid CSV.'); }
  }

  async function importData() {
    if (!file || !rows.length || validation?.missing.length) return
  ...
  return <main className="min-h-screen bg-[#f7f9fc] px-5 py-8 text-[#08111f] sm:px-8 sm:py-10"><div className="mx-auto max-w-[1080px]">
    ...
    <div className="mt-5 border-b border-[#dbe2ec] pb-7"><p className="text-[11px] font-extrabold uppercase tracking-[.18em] text-[#1769e0]">Data ingestion</p><h1 className="mt-2 text-3xl font-black tracking-[-.04em]">Connect operational data</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#66758a]">Import records from your organization. The server validates the file and enforces workspace authorization before writing anything.</p></div>
